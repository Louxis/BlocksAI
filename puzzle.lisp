...
(defun square-1x1 (x y board)
  (place-square x y board))

(defun square-2x2(x y board)
         (labels ((square-aux (x y board cells) 
                  (if (null cells) (place-square x y board) 
                    (square-aux (first (first cells)) (second (first cells)) (place-square x y board) (cdr cells))))) (square-aux x y board (block-occupied-cells x y 'square-2x2))))
...
(defun solution-nodep (node expandf)
  (cond ((equal (node-pieces node) '(0 0 0)) t)
        ((null (funcall expandf node)) t)
        (t nil)))
  ...
  (flet ((expand-node (node operation)
           (let ((positions (possible-block-positions (node-board (node-state node)) operation)))
             (mapcar #'(lambda (position) 
                         (print-board (funcall operation (first position) (second position) (node-board (node-state node))))) positions))))
